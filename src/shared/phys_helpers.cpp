...
{
	GRAPHICS->draw(nullptr, nullptr, skygfx::utils::MeshBuilder::Mode::Lines, {
		{ .pos = { p1.x, p1.y, 0.0f }, .color = *(glm::vec4*)&color },
		{ .pos = { p2.x, p2.y, 0.0f }, .color = *(glm::vec4*)&color }
	});
}
...
void World::Draw::DrawTransform(const b2Transform& xf)
{
	...
	GRAPHICS->draw(nullptr, nullptr, skygfx::utils::MeshBuilder::Mode::Lines, {
		{ .pos = { p.x, p.y, 0.0f }, .color = color1 },
		{ .pos = { px.x, px.y, 0.0f }, .color = color1 },
		{ .pos = { p.x, p.y, 0.0f }, .color = color2 },
		{ .pos = { py.x, py.y, 0.0f }, .color = color2 }
	});
}
