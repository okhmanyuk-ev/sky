...
namespace Shared::SceneHelpers
{
	...
	// TODO: think about naming, maybe better TabManager
	// TODO: think about moving to scene
	class PagesManager
	{
	public:
		using PageChangedCallback = std::function<void(std::optional<size_t> from_index, size_t to_index)>;

	public:
		class Page;

	public:
		// returning page index
		size_t addPage(std::shared_ptr<Page> page);
	...
	private:
		...
		std::optional<size_t> mCurrentPage;
	
	public:
		bool isCarousel() const { return mCarousel; }
		void setCarousel(bool value) { mCarousel = value; }

		auto getPageChangedCallback() const { return mPageChangedCallback; }
		void setPageChangedCallback(PageChangedCallback value) { mPageChangedCallback = value; }

	private:
		bool mCarousel = true;
		PageChangedCallback mPageChangedCallback = nullptr;
	};

	class PagesManager::Page
	{
	public:
		virtual void onShow(std::optional<size_t> from_index, size_t to_index) = 0;
		virtual void onHide(size_t from_index, size_t to_index) = 0;
	};

	template <typename T> class MappedPagesManager : public PagesManager
	{
		static_assert(std::is_enum<T>::value, "T must be enum");
	public:
		void addPage(T type, std::shared_ptr<Page> page)
		{
			mIndicesMap.insert({ type, PagesManager::addPage(page) });
		}

		void showPage(T type)
		{
			PagesManager::showPage(mIndicesMap.at(type));
		}

	private:
		std::map<T, int> mIndicesMap;
	};
}