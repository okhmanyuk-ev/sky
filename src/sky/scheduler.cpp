...
void Scheduler::frame()
{
	...
	if (auto framerateLimit = mFramerateLimit.getValue(); framerateLimit > 0)
	{
		auto frameTime = sky::FromSeconds(1.0 / framerateLimit);
		auto sleepAllowed = mSleepAllowed.getValue();
